...
class EP(EPBase, ExactGaussianInference):
    ...
    def _inference(self, Y, K, ga_approx, likelihood, Z_tilde, Y_metadata=None):
        ...
        dL_dthetaL = likelihood.ep_gradients(Y, ga_approx.tau, ga_approx.v, Y_metadata=Y_metadata, dL_dKdiag=np.diag(dL_dK))
        #exact_inference_gradients(np.diag(dL_dK), Y_metadata)
